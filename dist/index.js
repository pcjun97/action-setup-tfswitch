...
var owner = "warrensbox";
var repo = "terraform-switcher";
...
function getAsset(release, platform, arch, tag) {
  ...
  const version2 = release.data.tag_name;
  const name = `terraform-switcher_${version2}_${platform}_${arch}.${suffix}`;
  const asset = release.data.assets.find((asset2) => asset2.name === name);
  if (!asset) {
    throw new Error(
      `platform ${platform}/${arch} is not supported for version ${version2}. Please file a request at https://github.com/${owner}/${repo}/issues/new`
    );
  }
  return asset;
}
...
async function run() {
  try {
    ...
    core.info(`found version: ${version2} for ${tag}/${platform}/${arch}`);
